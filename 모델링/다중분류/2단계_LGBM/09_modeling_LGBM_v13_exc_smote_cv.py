# ==========================================================
# 09_modeling_LGBM_v13_exc_smote_cv.py
# Stage1 vs Stage2+3 | Excluded 28개 | SMOTE+CV
# ==========================================================
...
OUT_DIR = "outputs/LGBM_v13_exc_smote_cv"
...
REMOVE_COLS = [
    'urine_output_sum','urine_output_6h','oliguria_flag',
    'creatinine_min','creatinine_max','creatinine_delta','bun_cr_ratio'
]
keep_idx = [i for i,col in enumerate(ORIGINAL_FEATURE_COLS) if col not in REMOVE_COLS]
FEATURE_COLS = [col for col in ORIGINAL_FEATURE_COLS if col not in REMOVE_COLS]

print("=" * 70)
print("LGBM v13 | Excluded 28개 | SMOTE+CV | Stage1 vs Stage2+3")
print("=" * 70)
...
print(f"After SMOTE: Stage1={sum(y_sm==0)}, Stage2+3={sum(y_sm==1)}")
...
def objective(trial):
    ...
    scores=[]
    for tr_idx,val_idx in cv.split(X_sm,y_sm):
        m=LGBMClassifier(**params)
        m.fit(X_sm[tr_idx],y_sm[tr_idx])
        pred=m.predict(X_sm[val_idx])
        s23r=recall_score(y_sm[val_idx],pred,labels=[1],average=None)[0]
        macro=f1_score(y_sm[val_idx],pred,average="macro")
...
print("OPTUNA SMOTE+CV (50 trials)")
...
print(f"AUROC={val_auroc:.4f} | S1={val_s1r:.4f} | S2+3={val_s23r:.4f}")
print()
...
plt.title("Confusion Matrix (v13 exc smote_cv)"); plt.tight_layout()
...
sns.heatmap(cm_pct,annot=True,fmt='.1%',cmap='Blues',
            xticklabels=["Stage1","Stage2+3"],yticklabels=["Stage1","Stage2+3"])
plt.title("Normalized CM (v13 exc smote_cv)"); plt.tight_layout()
...
plt.title("ROC Curve (v13 exc smote_cv)")
...
plt.title("Performance (v13 exc smote_cv)"); plt.tight_layout()
...
plt.title("Top 15 (v13 exc smote_cv)")
...
with open("models/stage2_LGBM_v13_exc_smote_cv.pkl","wb") as f:
    pickle.dump({"model":model,"feature_cols":FEATURE_COLS,
                 "remove_cols":REMOVE_COLS,"best_threshold":best_t,
                 "version":"v13_exc_smote_cv",
                 "metrics":{"valid_auroc":val_auroc,"valid_s23r":val_s23r,
                             "test_auroc":test_auroc,"test_s23r":test_s23r}},f)
print("MODEL SAVED : models/stage2_LGBM_v13_exc_smote_cv.pkl")
print("FINISHED")