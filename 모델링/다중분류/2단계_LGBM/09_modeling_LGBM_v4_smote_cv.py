...
# ==========================================================
# 09_modeling_LGBM_v4_smote_cv.py
#
...
OUT_DIR = "outputs/LGBM_stage_v4_smote_cv"
...
X_sm, y_sm = smote.fit_resample(X_stage_train, y_tr)

print("After SMOTE:")
print(pd.Series(y_sm).value_counts().sort_index()
      .rename({0:"Stage1",1:"Stage2",2:"Stage3"}))
print(f"Train shape: {X_sm.shape}")
print()

# ==========================================================
# OPTUNA — SMOTE 데이터로 5-Fold CV
# ==========================================================
...
print("=" * 70)
print("OPTUNA TUNING v4 SMOTE+CV (50 trials)")
print("=" * 70)
...
plt.title("Per-class Recall (LGBM v4 SMOTE+CV)")
plt.ylabel("Recall")
plt.legend()
plt.tight_layout()
...
plt.title("Optuna Trial History (LGBM v4 SMOTE+CV)")
plt.tight_layout()
...
plt.suptitle("SMOTE 전후 Stage 분포 비교 (v4)")
plt.tight_layout()
...
with open("models/stage2_LGBM_v4_smote_cv.pkl", "wb") as f:
    pickle.dump({
        "model"       : model,
...
print("=" * 70)
print("MODEL SAVED : models/stage2_LGBM_v4_smote_cv.pkl")
print("=" * 70)
print("FINISHED")