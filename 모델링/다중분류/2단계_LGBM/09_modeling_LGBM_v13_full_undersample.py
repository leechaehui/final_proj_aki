# ==========================================================
# 09_modeling_LGBM_v13_full_undersample.py
# Stage1 vs Stage2+3 | Full 35개 | Undersampling
# ==========================================================
...
OUT_DIR = "outputs/LGBM_v13_full_undersample"
...
print("=" * 70)
print("LGBM v13 | Full 35개 | Undersample | Stage1 vs Stage2+3")
print("=" * 70)
...
rus = RandomUnderSampler(
    sampling_strategy={0:n_s23, 1:n_s23}, random_state=42)
...
for fname,data,title in [
    ("confusion_matrix.png",cm,"Confusion Matrix (v13 full undersample)"),
]:
    ...
    plt.savefig(f"{OUT_DIR}/{fname}",dpi=300); plt.close()
...
sns.heatmap(cm_pct,annot=True,fmt='.1%',cmap='Blues',
            xticklabels=["Stage1","Stage2+3"],
            yticklabels=["Stage1","Stage2+3"])
plt.title("Normalized CM (v13 full undersample)")
...
plt.title("ROC Curve (v13 full undersample)")
...
plt.title("Performance Summary (v13 full undersample)")
plt.tight_layout(); plt.savefig(f"{OUT_DIR}/performance_summary.png",dpi=300); plt.close()
...
plt.title("Top 15 Feature Importance (v13 full undersample)")
...
with open("models/stage2_LGBM_v13_full_undersample.pkl","wb") as f:
    pickle.dump({"model":model,"feature_cols":FEATURE_COLS,
                 "best_threshold":best_t,"version":"v13_full_undersample",
                 "metrics":{"valid_auroc":val_auroc,"valid_s23r":val_s23r,
                             "test_auroc":test_auroc,"test_s23r":test_s23r}},f)
print("MODEL SAVED : models/stage2_LGBM_v13_full_undersample.pkl")
print("FINISHED")