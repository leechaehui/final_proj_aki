# ==========================================================
# 09_modeling_LGBM_v11_undersample.py
#
# Stage 2 : AKI Stage Classification
# Method : Random Undersampling
#           Stage1/3을 Stage2 수(701명)로 균등화
...
OUT_DIR = "outputs/LGBM_stage_v11_undersample"
...
print(before.rename({0:"Stage1",1:"Stage2",2:"Stage3"}))
print()

# Stage2 수(701명)에 맞춰 Stage1, Stage3 줄임
n_stage2 = int(sum(y_tr == 1))
sampling_strategy = {
    0: n_stage2,  # Stage1 → 701명
    1: n_stage2,  # Stage2 → 701명 (그대로)
    2: n_stage2,  # Stage3 → 701명
}
...
X_us, y_us = rus.fit_resample(X_stage_train, y_tr)

print("After Undersampling:")
after = pd.Series(y_us).value_counts().sort_index()
print(after.rename({0:"Stage1",1:"Stage2",2:"Stage3"}))
print(f"Total: {len(y_us)}명 (기존 {len(y_tr)}명에서 {len(y_tr)-len(y_us)}명 제거)")
print()
...
axes[1].bar(["Stage1","Stage2","Stage3"], after.values,
            color=["#185FA5","#E24B4A","#E67E22"])
axes[1].set_title("After Undersampling (균등)")
axes[1].set_ylabel("Count")
for i, v in enumerate(after.values):
    axes[1].text(i, v+10, str(v), ha='center', fontsize=12)
plt.suptitle("Undersampling 전후 Stage 분포 비교")
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/undersample_distribution.png", dpi=300)
plt.close()
...
print("=" * 70)
print("OPTUNA TUNING v11 Undersample (50 trials)")
print("=" * 70)
...
plt.title("Per-class Recall (LGBM v11 Undersample)")
plt.ylabel("Recall")
plt.legend()
plt.tight_layout()
...
plt.xlabel("Trial")
plt.ylabel("Macro F1")
plt.title("Optuna Trial History (LGBM v11 Undersample)")
plt.tight_layout()
...
with open("models/stage2_LGBM_v11_undersample.pkl", "wb") as f:
    pickle.dump({
        "model"       : model,
        "feature_cols": FEATURE_COLS,
        "remove_cols" : REMOVE_COLS,
        "best_params" : best_params,
        "version"     : "v11_undersample",
        "n_per_class" : n_stage2,
        "metrics": {
...
print("=" * 70)
print("MODEL SAVED : models/stage2_LGBM_v11_undersample.pkl")
print("=" * 70)
print("FINISHED")
