# ==========================================================
# 09_modeling_LGBM_v13_exc_undersample.py
# Stage1 vs Stage2+3 | Excluded 28개 | Undersampling
# ==========================================================
...
OUT_DIR = "outputs/LGBM_v13_exc_undersample"
...
print("=" * 70)
print("LGBM v13 | Excluded 28개 | Undersample | Stage1 vs Stage2+3")
print("=" * 70)
...
n_s23 = int(sum(y_tr==1))
print(f"Before: Stage1={sum(y_tr==0)}, Stage2+3={sum(y_tr==1)}")
rus = RandomUnderSampler(sampling_strategy={0:n_s23,1:n_s23}, random_state=42)
X_us, y_us = rus.fit_resample(X_tr, y_tr)
print(f"After:  Stage1={sum(y_us==0)}, Stage2+3={sum(y_us==1)}")
...
plt.title("Confusion Matrix (v13 exc undersample)"); plt.tight_layout()
...
sns.heatmap(cm_pct,annot=True,fmt='.1%',cmap='Blues',
            xticklabels=["Stage1","Stage2+3"],yticklabels=["Stage1","Stage2+3"])
plt.title("Normalized CM (v13 exc undersample)"); plt.tight_layout()
...
plt.title("ROC Curve (v13 exc undersample)")
...
plt.title("Performance (v13 exc undersample)"); plt.tight_layout()
...
plt.title("Top 15 (v13 exc undersample)")
