# v5: 수동 가중치 조정
#     Stage2 가중치를 더 강하게 설정
#     {0: 0.43, 1: 5.0, 2: 3.5}
#     Stage2 Recall 최대화 목적

# ==========================================================
# 09_modeling_LGBM_v5_manual_weight.py
#
...
OUT_DIR = "outputs/LGBM_stage_v5_manual_weight"
...
# ==========================================================
# 수동 가중치 설정 — Stage2 극단 강조
# ==========================================================

print("=" * 70)
print("MANUAL CLASS WEIGHT (Stage2 극단 강조)")
print("=" * 70)

# 여러 가중치 조합 시도
WEIGHT_CONFIGS = [
    {"name": "config_A", "weights": {0: 0.30, 1: 10.0, 2: 1.50}},
    {"name": "config_B", "weights": {0: 0.30, 1: 15.0, 2: 1.50}},
    {"name": "config_C", "weights": {0: 0.30, 1: 20.0, 2: 1.00}},
]

for cfg in WEIGHT_CONFIGS:
    print(f"  {cfg['name']}: "
          f"Stage1={cfg['weights'][0]}, "
          f"Stage2={cfg['weights'][1]}, "
          f"Stage3={cfg['weights'][2]}")
print()

# config_B 채택 (Stage2 가중치 15배)
MANUAL_WEIGHT = {0: 0.30, 1: 15.0, 2: 1.50}
sample_weight = np.array([MANUAL_WEIGHT[y] for y in y_tr])

print(f"채택 가중치: Stage1={MANUAL_WEIGHT[0]}, "
      f"Stage2={MANUAL_WEIGHT[1]}, "
      f"Stage3={MANUAL_WEIGHT[2]}")
print()

print("Stage 분포 (Train)")
...
def objective(trial):
    ...
    # Stage2 Recall 직접 최대화
    s2_recall = recall_score(
        y_vl, pred, labels=[1], average=None
    )[0]

    # Macro F1도 함께 고려 (Stage2 Recall + Macro F1 조합)
    macro_f1  = f1_score(y_vl, pred, average="macro")
    score     = s2_recall * 0.7 + macro_f1 * 0.3

    trial_history.append([trial.number, s2_recall, macro_f1, score])
    print(f"Trial {trial.number:03d} | "
          f"S2 Recall={s2_recall:.4f} | "
...
print("=" * 70)
print("OPTUNA TUNING v5 (50 trials, Stage2 Recall 최대화)")
print("=" * 70)
...
trial_df = pd.DataFrame(
    trial_history,
    columns=["trial","s2_recall","macro_f1","score"]
)
...
plt.title("Per-class Recall (LGBM v5 Manual Weight)")
plt.ylabel("Recall")
plt.legend()
plt.tight_layout()
...
# Optuna History — S2 Recall + Macro F1 + Score
plt.figure(figsize=(10, 5))
plt.plot(trial_df["trial"], trial_df["s2_recall"],
...
plt.plot(trial_df["trial"], trial_df["score"],
         label="Score (0.7×S2R + 0.3×F1)", color="#E67E22",
         marker="o", markersize=3, linestyle="--")
plt.xlabel("Trial")
plt.ylabel("Score")
plt.title("Optuna Trial History (LGBM v5 Manual Weight)")
plt.legend()
plt.tight_layout()
...
with open("models/stage2_LGBM_v5_manual_weight.pkl", "wb") as f:
    pickle.dump({
        "model"        : model,
        "feature_cols" : FEATURE_COLS,
        "remove_cols"  : REMOVE_COLS,
        "best_params"  : best_params,
        "version"      : "v5_manual_weight",
...
print("=" * 70)
print("MODEL SAVED : models/stage2_LGBM_v5_manual_weight.pkl")
print("=" * 70)
print("FINISHED")