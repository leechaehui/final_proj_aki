...
def objective(trial):
    ...
    trial_history.append([trial.number, score])
    print(f"Trial {trial.number:03d} | AUPRC = {score:.5f}")
    return score

print("=" * 70)
print("OPTUNA TUNING v1 (30 trials, 넓은 범위)")
print("=" * 70)

study = optuna.create_study(direction="maximize")
study.optimize(objective, n_trials=30, show_progress_bar=False)

trial_df = pd.DataFrame(trial_history, columns=["trial", "auprc"])
...
disp.plot()
plt.title("Validation Confusion Matrix (RF v1 Full)")
plt.tight_layout()
...
plt.ylabel("Actual")
plt.title("Normalized Confusion Matrix (RF v1 Full)")
plt.tight_layout()
...
plt.title("ROC Curve (RF v1 Full)")
plt.legend()
plt.tight_layout()
...
plt.title(f"PR Curve (RF v1 Full) | AUPRC={val_auprc:.4f}")
plt.tight_layout()
...
plt.ylabel("Count")
plt.title("Probability Distribution (RF v1 Full)")
plt.legend()
plt.tight_layout()
...
plt.ylabel("Score")
plt.title("Threshold Optimization (RF v1 Full)")
plt.legend()
plt.tight_layout()
...
plt.title("Validation Performance Summary (RF v1 Full)")
plt.ylabel("Score")
plt.tight_layout()
...
plt.ylabel("AUROC")
plt.title("Train / Validation / Test AUROC (RF v1 Full)")
plt.tight_layout()
...
plt.title("Top 15 Feature Importance (RF v1 Full)")
plt.tight_layout()
...
# 10. Optuna History
plt.figure(figsize=(8, 5))
plt.plot(trial_df["trial"], trial_df["auprc"],
         marker="o", markersize=4, color="#185FA5")
plt.xlabel("Trial")
plt.ylabel("AUPRC")
plt.title("Optuna Trial History (RF v1 Full)")
plt.tight_layout()
...
with open("models/stage1_RF_v1_full.pkl", "wb") as f:
    pickle.dump({
        "model"         : model,
...
print("=" * 70)
print("MODEL SAVED : models/stage1_RF_v1_full.pkl")
print("=" * 70)
print("FINISHED")
print()
print("※ 주의: v1은 넓은 탐색 범위로 과적합 가능성 있음")
print("   Valid AUROC 0.99 이상이면 v2_full 또는 cv_full 사용")